...
for t in range(ds.dims['count']):
    ...
    heights, line_coords = draw_line(height_map, 179, 128, 128)
    mean_heights_frame = []
    for k in range(len(line_coords[0])):
        orthogonal_heights, _ = draw_line(height_map, 179+90, line_coords[0][k], line_coords[1][k])
...
model_fit = ARIMA(smoothed_imu[:400], exog=mean_heights_np[:400, :], order=(1, 0, 0)).fit()
val=0
next=4
fer=0
...
plt.plot(x_values[400:], smoothed_pred, label='Predicted IMU')  # Start plotting predictions from x=35
plt.legend()
plt.show()
